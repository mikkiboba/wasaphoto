...
func (db *appdbimpl) GetStream(followingList []string) ([]Post, error) {
	...
	var posts []Post
	rows, err := db.c.Query(`SELECT id, user, date, hour FROM posts WHERE user IN (` + followingS + `) ORDER BY date DESC, hour DESC`)
	if err != nil {
		return nil, err
	}
...
func (db *appdbimpl) GetPosts(uid string) ([]Post, error) {
	...
	var posts []Post
	rows, err := db.c.Query(`SELECT id, user, date, hour FROM posts WHERE user = ? ORDER BY date, hour DESC`, uid)
	if err != nil {
		return nil, err
	}
